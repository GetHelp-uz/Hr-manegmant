...
router.get("/attendance-summary", requireAuth, async (req, res) => {
  try {
    ...
    const summaries = await Promise.all(employees.map(async (emp) => {
      ...
      const lateDays = attendanceRecords.filter(a => (a.lateMinutes || 0) > 0).length;
      const totalHours = attendanceRecords.reduce((sum, a) => sum + (a.workHours ? parseFloat(a.workHours.toString()) : 0), 0);

      const checkInTimes = attendanceRecords.filter(a => a.checkIn).map(a => {
      ...
      return {
        employee: {
          id: emp.id, companyId: emp.companyId, fullName: emp.fullName, phone: emp.phone,
          position: emp.position, salaryType: emp.salaryType,
          ...
          qrCode: emp.qrCode, telegramId: emp.telegramId, createdAt: emp.createdAt,
        },
